      ...
      try {
        ...
        try {
          ...
          if (process.env.NODE_ENV === 'development') {
            return res.status(200).json({ 
              message: 'Reset email sent successfully! (Development mode)',
              success: true,
              resetUrl: resetUrl
            });
          }
        ...
        return res.status(200).json({ 
          message: 'Reset email sent successfully! (Fallback mode)',
          success: true,
          resetUrl: resetUrl
        });
      }
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          return res.status(200).json({ 
            message: 'If this email exists in our system, you will receive a reset link shortly. (Development mode)',
            success: true,
            resetUrl: resetUrl
          });
        }
